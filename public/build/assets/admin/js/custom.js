(function ($) {
	...
	$.get("https://hamariweb.com/finance/forex/open_market_rates.aspx", function(data){
		var page_context = $(data);
		var table_context = $(page_context.find(".table"));
		table_context.find("a").attr("href", "#");
		table_context.find("img").remove();
		var count = 1;
		table_context.find("tbody").find("tr").each(function(){
			if(count > 7){
				$(this).remove();
			}
			count += 1;
		})
		$("#currency_table").html(table_context.html());
	});
	//  Full screen
